class GiftGuideManager {
  constructor() {
      ...
      this.softWinterJacketVariantId = 1/* Replace with the actual variant ID */;
  ...
  renderPopupContent(product) {
      ...
      form.addEventListener('submit', (event) => {
          ...
          this.addToCart(variantId, quantity, product);
      });
  }

  async addToCart(variantId, quantity, product) {
      try {
          ...
          const cartProduct = product.variants.find(variant => variant.id == variantId);

          if (cartProduct.option1 === "Black" && cartProduct.option2 === "Medium") {
...
// REMOVED addSoftWinterJacket function entirely
}
